...
import { X, MapPin, Clock, Star, CheckCircle2 } from "lucide-react";
...
export default function PackageDetailsModal({ pkg, onClose }) {
  ...
  // When booking is open, render it INSTEAD of the details modal
  if (showBooking) {
    return <BookingModal pkg={pkg} onClose={onClose} />;
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
      <div className="relative w-full max-w-4xl bg-white rounded-[2rem] shadow-2xl overflow-hidden my-auto max-h-[90vh] flex flex-col">
        ...
        <div className="relative h-64 sm:h-80 shrink-0">
          <img src={pkg.imageURL} alt={pkg.title} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
          ...
          <div className="absolute bottom-6 left-6 right-6">
            ...
            <h2 className="text-3xl sm:text-4xl font-serif text-white mb-2 leading-tight">{pkg.title}</h2>
            <div className="flex items-center gap-4 text-white/90 text-sm font-medium">
              <div className="flex items-center gap-1.5"><MapPin className="h-4 w-4 text-orange-400" />{pkg.location}</div>
              <div className="flex items-center gap-1.5"><Clock className="h-4 w-4 text-orange-400" />{pkg.duration}</div>
              <div className="flex items-center gap-1.5"><Star className="h-4 w-4 fill-yellow-400 text-yellow-400" />4.9</div>
            </div>
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-3 gap-10">
                  ...
                  <div className="relative border-l border-slate-200 ml-3 space-y-8">
                    {pkg.itinerary.map((day, i) => (
                      <div key={i} className="relative pl-8">
                        ...
                        <p className="text-slate-600 leading-relaxed">{day.activities}</p>
                      </div>
                    ))}
                  </div>
                </section>
            ...
            <div className="md:col-span-1">
              <div className="sticky top-0 bg-slate-50 p-6 rounded-3xl border border-slate-100 shadow-sm">
                <div className="mb-6">
                  <span className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Price per person</span>
                  <div className="text-3xl font-bold text-slate-900">{formatCurrency(pkg.price)}</div>
                </div>
                <div className="space-y-3 mb-8">
                  {["Premium Accommodation", "Expert Local Guide", "Daily Breakfast", "Airport Transfers"].map(item => (
                ...
                <button
                  ...
                  className="w-full bg-orange-600 hover:bg-orange-700 active:scale-[0.98] text-white font-bold py-4 rounded-xl transition-all shadow-lg shadow-orange-200 mb-3"
                >
                  Book This Tour
                </button>
