    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-end items-start sm:items-center gap-4">
        <button
          ...
          className="w-full sm:w-auto bg-orange-600 hover:bg-orange-700 text-white px-6 py-3 rounded-2xl font-bold transition-all shadow-xl shadow-orange-600/20 flex items-center justify-center gap-2 active:scale-95"
        >
      ...
      <div className="overflow-x-auto pb-1 -mx-4 px-4 sm:mx-0 sm:px-0 scrollbar-hide">
        <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-2xl w-fit whitespace-nowrap">
          {[
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
        {filteredItems.map((item) => (
          ...
          >
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-all duration-300 flex flex-col justify-between p-3 sm:p-4">
              <div className="flex justify-between items-start">
                <div className="bg-white/20 backdrop-blur-md p-1.5 rounded-lg text-white border border-white/10">
                  {item.mediaType === "video" ? <Film className="h-3.5 w-3.5 sm:h-4 w-4" /> : <ImageIcon className="h-3.5 w-3.5 sm:h-4 w-4" />}
                </div>
                <div className="flex gap-1.5">
                  <button
                    onClick={() => handleOpenModal(item)}
                    className="bg-white/90 backdrop-blur-md text-slate-900 p-2 rounded-xl hover:bg-white transition-all shadow-lg active:scale-90"
                  >
                    <Edit3 className="h-3.5 w-3.5 sm:h-4 w-4" />
                  </button>
                  <button
                    onClick={() => handleDelete(item.id)}
                    className="bg-red-500/90 backdrop-blur-md text-white p-2 rounded-xl hover:bg-red-600 transition-all shadow-lg active:scale-90"
                  >
              ...
              <div className="space-y-1">
                <span className="inline-block px-2 py-0.5 bg-orange-600 text-white text-[9px] sm:text-[10px] font-black uppercase tracking-wider rounded-md mb-1">{item.category}</span>
                {item.title && <h4 className="text-white font-bold text-xs sm:text-sm truncate leading-tight">{item.title}</h4>}
                {item.description && <p className="text-white/70 text-[10px] sm:text-xs line-clamp-1 mt-0.5">{item.description}</p>}
                <a
                  href={item.mediaUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 text-white/50 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider hover:text-white mt-1 transition-colors"
                >
                  View Original <ExternalLink className="h-2.5 w-2.5 sm:h-3 w-3" />
                </a>
              </div>
            </div>
          </div>
      ...
      {isModalOpen && (
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200">
            <div className="sticky top-0 bg-white/80 backdrop-blur-md border-b border-slate-100 px-5 sm:px-6 py-4 flex items-center justify-between z-10">
              <h2 className="text-lg sm:text-xl font-bold text-slate-900">{editingItem ? "Edit Media" : "Add Media"}</h2>
              <button onClick={handleCloseModal} className="p-2 text-slate-400 hover:bg-slate-100 rounded-full transition-all active:scale-90">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                    <div className="flex-1">
                      <input 
                        ...
                        className="w-full px-4 py-3 sm:py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500 transition-all text-sm" 
                        placeholder="Paste image/video URL here..." 
                        required={!formData.mediaUrl.startsWith('data:')} 
                      />
                    </div>
                    <div className="flex items-center justify-center relative py-1 sm:py-0 px-2">
                      <div className="hidden sm:block h-8 w-px bg-slate-200 mx-2" />
                      <span className="text-[10px] font-black text-slate-400 bg-white px-2 py-1 rounded-md border border-slate-100">OR</span>
                      <div className="hidden sm:block h-8 w-px bg-slate-200 mx-2" />
                    </div>
                    <div className="relative flex-1">
                      ...
                      />
                      <div className="flex items-center justify-center w-full h-full px-4 py-3 sm:py-2.5 rounded-xl border border-dashed border-slate-300 bg-slate-50 text-slate-500 text-sm font-bold hover:bg-slate-100 transition-all truncate">
                ...
                {formData.mediaType === "video" && (
                  <div className="space-y-1.5 pt-4 border-t border-slate-100">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                      <input type="url" value={formData.thumbnailUrl.startsWith('data:') ? '' : formData.thumbnailUrl} onChange={e => setFormData({...formData, thumbnailUrl: e.target.value})} className="flex-1 px-4 py-3 sm:py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500 transition-all text-sm" placeholder="https://..." />
                      <div className="flex items-center justify-center relative py-1 sm:py-0 px-2">
                        <span className="text-[10px] font-black text-slate-400 bg-white px-2 py-1 rounded-md border border-slate-100">OR</span>
                      </div>
                      <div className="relative flex-1">
              ...
              <div className="flex flex-col sm:flex-row justify-end gap-3 pt-6 border-t border-slate-100">
                <button type="button" onClick={handleCloseModal} className="w-full sm:w-auto px-5 py-3 sm:py-2.5 text-sm font-bold text-slate-600 hover:bg-slate-50 rounded-xl transition-all active:scale-95">
                  Cancel
                </button>
                <button 
                  type="submit" 
                  disabled={isLoading}
                  className="w-full sm:w-auto px-5 py-3 sm:py-2.5 text-sm font-bold text-white bg-orange-600 hover:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-xl transition-all shadow-xl shadow-orange-600/20 flex items-center justify-center gap-2 active:scale-95"
                >
